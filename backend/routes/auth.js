...
router.post('/forgot-password', async (req, res) => {
    ...
    // 1. Check user exists in our users table
    ...
    // 3. Generate recovery link via Supabase Admin API (does NOT send email)
    ...
    // 4. Send email ourselves via Brevo SMTP
    try {
            ...
            html: `
                <div style="font-family: Arial, sans-serif; max-width: 480px; margin: 0 auto; background: #0a0a0a; padding: 0;">
                    ...
                    <div style="padding: 32px; color: #f5f0e8;">
                        <h2 style="font-size: 18px; margin: 0 0 16px; color: #FFE000; text-transform: uppercase;">
                            Password Reset
                        </h2>
                        <p style="font-size: 14px; line-height: 1.7; margin: 0 0 8px; color: #ccc;">
                            Hi <strong style="color: #FFE000;">${ourUser.username}</strong>,
                        </p>
...
router.post('/reset-password', async (req, res) => {
    ...
    try {
        // 1. Verify the Supabase access token to get the user
        ...
        // 3. Update password hash in our users table too (for custom login)
